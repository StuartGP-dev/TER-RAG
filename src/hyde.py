...
bnb_config = BitsAndBytesConfig(
    ...
    bnb_4bit_compute_dtype=torch.bfloat16 # Accélère le calcul 
)
...
def retrieve(query: str) -> pd.DataFrame:
    """
    Génère un document hypothétique avec HyDE, puis recherche les chunks les plus proches sémantiquement.
    """
    ...
    # 2. Encodage et calcul de la similarité cosinus
...
def retrieve_batch(queries: list[str]) -> list[pd.DataFrame]:
    """
    Traite plusieurs requêtes en une seule passe afin de réduire le temps d'exécution.
    """
    batch_size = 4
    ...
    # 2. Génération groupée des documents hypothétiques
    outputs = generator(
        all_prompts, 
        max_new_tokens=100, 
